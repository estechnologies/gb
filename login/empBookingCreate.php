			...
			if($timeOk){ 
					...
					if( $result == true){
						...
						sendMail($Email,$hashId);
												header("Location:BookingCreate.php?msg1=Your Booking has been Successful. Your Booking id is $hashId.<br>Our Support Team will contact you shortly.");

					}else{
						header("Location:BookingCreate.php?msg=booing failed please contact goodscab");
					}
			}else{
				header("Location:BookingCreate.php?msg=Pickup Date and Time should be at least 2 hours from the current time");
			}
		}else{
			header("Location:BookingCreate.php?msg=Shipment weight should be greater than zero");
		}
		...
		function  sendMail($email,$bookingId){
			...
			*/
			...
			if(!$mail->send()) {
				...
				header("Location:BookingCreate.php?msg=".$error);
					
			} else {
				header("Location:BookingCreate.php?msg=email successfully sent if not found check spam folder");
			}
