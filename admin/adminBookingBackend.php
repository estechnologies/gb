<?php
		...
		if(courierCheck(intval($Shipment_weight), $Shipment_Type) == true){
			if($timeOk){ 
					...
					$sql = "INSERT INTO goodscab_Booking ".
							"(from_Address,to_Address,booking_time,Travel_Date_Time,Marterial_Description,Shipment_weight,Num_package,Delivery_Type,Shipment_Type,Email,usrtel,distance,travel_data,Vehicle_Capacity,type,status) ".
							"VALUES ".
							"('$from_Address','$to_Address',NOW(),'$Travel_Date_Time','$Marterial_Description','$Shipment_weight','$Num_package','$Delivery_Type','$Shipment_Type','$Email','$usrtel','$distance','$travel_data','$Vehicle_Capacity','order','Confirmation Pending')";
					...
					if( $result == true){
						...
						sendMail($Email,$hashId);
												header("Location:AdminBookingCreate.php?msg1=Your Booking has been Successful. Your Booking id is $hashId.<br>Our Support Team will contact you shortly.");

					}else{
						header("Location:AdminBookingCreate.php?msg=booing failed please contact goodscab");
					}
			}else{
				header("Location:AdminBookingCreate.php?msg=Pickup Date and Time should be at least 2 hours from the current time");
			}
		}else{
			header("Location:AdminBookingCreate.php?msg=Shipment weight should be greater than zero");
		}
		...
		/*
		 * booking id sent to user email
		 */
		function  sendMail($email,$bookingId){
			...
			*/
			...
			if(!$mail->send()) {
				...
				header("Location:AdminBookingCreate.php?msg=".$error);
					
			} else {
				header("Location:AdminBookingCreate.php?msg=email successfully sent if not found check spam folder");
			}
