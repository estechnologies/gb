<?php
	...
	function uploadfiles($count){
		...
		if($photo != null){
			...
			if(move_uploaded_file($photo_loc, $folder.$finalPhoto)){
				return "admin/".$folder.$finalPhoto;
			}else{
				return '';
			}
		}else{
	...
	if($resultQuery == 1){
		header("Location:admin_packagingSolution_customOrders_create.php?msg1=Your order has been confirmed. Your Order Id is $cdid.<br>Our Executive will get in touch with you for order processing.");
	}else{
		header("Location:admin_packagingSolution_customOrders_create.php?msg=Custom design update failed. Please try again.");
	}	
